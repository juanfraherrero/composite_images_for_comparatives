...
class CompositeImagesApp:
    def __init__(self, root):
        ...
        self.generate_button = tk.Button(root, text="Generate Columns", command=self.generate_file_fields)
        ...
        self.create_button = tk.Button(root, text="Create Composite", command=self.create_composite)
    ...
    def browse_output(self):
        output_file = filedialog.asksaveasfilename(defaultextension=".png", filetypes=[("PNG files", "*.png"), ("JPEG files", "*.jpg;*.jpeg")])
        if output_file:
            self.output_entry.delete(0, tk.END)  # Limpiar la entrada antes de insertar el nuevo archivo
    ...
    def generate_file_fields(self):
        ...
        try:
            ...
            messagebox.showerror("Error", "Por favor, ingresa un número válido de columnas.")
            return
        
        for i in range(columns):
            tk.Label(self.files_frame, text=f"Name:").grid(row=i, column=0, sticky="we")
            ...
            tk.Label(self.files_frame, text=f"Files:").grid(row=i, column=2, sticky="we")
            entry = tk.Entry(self.files_frame)
            entry.grid(row=i, column=3, sticky="we")
            browse_button = tk.Button(self.files_frame, text="Browse", command=lambda e=entry: self.browse_files(e))
