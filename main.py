...
images = [[crop_center(Image.open(image_path), (600,600)) for image_path in image_paths] for image_paths in config["images"]]

# Asumimos que todas las imágenes tienen el mismo tamaño
width, height = config["size"], config["size"]

cant_columns = len(images)
cant_rows =  len(max(images, key=len)) #obtener la mayopr cantidad de imagenes en una columnas
...
for index, column  in enumerate(column_names):
  draw.text(((width+spacing)*(index) + (width // 2) - draw.textsize(column, font=font)[0] // cant_columns, 0), column, fill="black", font=font) 

# Pegar las imágenes en la nueva imagen
for i in range(cant_rows):
  for j in range(cant_columns):
     new_image.paste(images[j][i], (j * (width + spacing) + spacing, column_name_height + spacing + i * (height + spacing)))
